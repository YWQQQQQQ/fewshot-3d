...
class Translation:
    def __init__(self, shift_range=0):
    ...
    def __call__(self, pcs):
        ...
        trans_matrix = torch.eye(pc_size[1] + 1, dtype=torch.float32)
        ...
        extra_one = torch.ones((pc_size[0], 1, pc_size[2]))
...
class Rotation:
    def __init__(self, angle_range=0):
        if angle_range < 0 or angle_range > 2*np.pi:
    ...
    def __call__(self, pcs):
        ...
        # x rotation
        x_rot_matrix = torch.eye(pc_size[1])
        x_theta = torch.rand(1) * 2 * self.angle_range
        ...
        # y rotation
        y_rot_matrix = torch.eye(pc_size[1])
        y_theta = torch.rand(1) * 2 * self.angle_range
        ...
        # z rotation
        z_rot_matrix = torch.eye(pc_size[1])
        z_theta = torch.rand(1) * 2 * self.angle_range
