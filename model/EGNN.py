...
class EdgeUpdateNetwork(nn.Module):
    def __init__(self, num_node_feats, device, ratio=[0.5], p=0):
    ...
    def forward(self, node_feats, edge_feats):
        ...
        force_edge_feat = torch.cat((torch.eye(num_samples).unsqueeze(0),
                                     torch.zeros(num_samples, num_samples).unsqueeze(0)), 0).unsqueeze(0).repeat(num_tasks, 1, 1, 1).to(self.device)
...
class NodeUpdateNetwork(nn.Module):
    def __init__(self, num_in_feats, num_node_feats, device, ratio=[2, 1], p=0):
